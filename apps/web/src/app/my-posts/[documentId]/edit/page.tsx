...
export default function EditMyPostPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(payload.error || "Cập nhật bài viết thất bại");
      }
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Sửa bài viết</h1>
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Cập nhật nội dung bài viết của bạn.</p>
      </header>
      ...
      >
        {loadingPost && <p className="text-sm text-zinc-500 dark:text-zinc-400">Đang tải bài viết...</p>}
